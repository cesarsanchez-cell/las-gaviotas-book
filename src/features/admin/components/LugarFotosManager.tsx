...
import {
  ...
  deleteLugarFotoAction,
  setLugarFotoPrincipalAction,
  updateLugarFotoAltAction,
} from "@/features/lugares/lib/foto-actions";
import type { LugarFotoRow } from "@/types/database";
...
export function LugarFotosManager({ lugarId, fotos, tipo }: Props) {
  ...
  async function handleFiles(files: FileList) {
    ...
    for (const file of Array.from(files)) {
      ...
      try {
        ...
        // Path: lugares/<id>/ dentro del bucket compartido `hospedajes`.
        const path = `lugares/${lugarId}/${Date.now()}-${safeName}`;
        ...
        const res = await registerLugarFotoAction({
          lugar_id: lugarId,
          storage_path: path,
          width,
          height,
  ...
  async function onDelete(foto: LugarFotoRow) {
    if (!confirm("¿Borrar esta foto? La acción no se puede deshacer.")) return;
    const res = await deleteLugarFotoAction({
      fotoId: foto.id,
      lugarId,
  ...
  async function onSetPrincipal(foto: LugarFotoRow) {
    const res = await setLugarFotoPrincipalAction({
      fotoId: foto.id,
      lugarId,
    });
    if (res.error) {
  ...
  async function onSaveAlt(foto: LugarFotoRow, alt: string) {
    const res = await updateLugarFotoAltAction({
      fotoId: foto.id,
      lugarId,
      alt,
    });
  ...
  const emptyHint =
    tipo === "gastronomico"
      ? "Subí fotos del local: frente, salón, platos, ambiente."
      : "Subí fotos del atractivo: panorámica, detalles, accesos.";
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {fotos.length === 0
              ...
              : `${fotos.length} ${
                ...
                }. La marcada como principal aparece en el listado y en la home si es imperdible.`}
          </p>
        </div>
        <label
          className={cn(
