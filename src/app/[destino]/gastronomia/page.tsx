...
export async function generateMetadata({
  ...
  return {
    title: `Gastronomía en ${destino.nombre}`,
    description: `Bares, restaurantes, parrillas, cafés y propuestas gastronómicas en ${destino.nombre}. Información, horarios y contacto directo.`,
    alternates: { canonical: `/${slug}/gastronomia` },
  };
}

export default async function GastronomiaListPage({
  params,
  searchParams,
}: PageProps) {
  const { destino: slug } = await params;
  ...
  const categoriaActiva =
    catParam && isCategoriaGastronomico(catParam) ? catParam : undefined;
    ...
    <>
      <JsonLd
        data={buildBreadcrumbJsonLd([
          ...
          { name: "Gastronomía", url: `/${slug}/gastronomia` },
        ])}
      />
          ...
          <Container size="xl">
            <header className="mb-8">
              ...
              <p className="mt-3 max-w-2xl text-muted-foreground">
                Bares, restaurantes y propuestas gastronómicas elegidas por la
                comunidad de {destino.nombre}.
              </p>
            </header>
            ...
            <div className="-mx-1 mb-8 flex flex-wrap gap-2">
              ...
              {CATEGORIAS_GASTRONOMICO_KEYS.map((key) => {
                const def = CATEGORIAS_GASTRONOMICO[key];
                const active = categoriaActiva === key;
                return (
                  <Link
                    key={key}
                    href={`/${slug}/gastronomia?categoria=${key}`}
