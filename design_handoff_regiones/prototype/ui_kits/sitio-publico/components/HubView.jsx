    ...
    <>
      ...
      >
        <div className="me-region-grid">
          {regiones.map((r) => (<RegionCard key={r.slug} region={r} onOpen={onOpenRegion} />))}
        </div>
      </Section>

      <NearbyBlock destinos={trending.slice(0, 4).concat(recientes.slice(0, 2))} onOpen={onOpenDestino} />
      ...
      >
        <div className="me-carousel">
          {trending.map((d) => (<DestinoMiniCard key={d.slug} destino={d} onOpen={onOpenDestino} />))}
        </div>
      </Section>
      ...
      >
        <div className="me-carousel">
          {recientes.map((d) => (<DestinoMiniCard key={d.slug} destino={d} onOpen={onOpenDestino} />))}
        </div>
      </Section>
